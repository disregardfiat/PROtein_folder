...
- **1‑hour wall‑clock timeout**: The server runs each HKE job in a **subprocess** with `start_new_session=True` and `timeout=PREDICTION_TIMEOUT_SEC` (default 3600). If the subprocess does not finish within that time, the **entire process group** is killed (so any child processes from the minimizer/JAX/threading also stop), and the server **sends the fast‑pass result** (the quick fold from Phase 1) by email with a note that the run hit the time limit. So the user always gets a result after at most 1 hour; no job can block indefinitely.
